...
class RepairWorker(QThread):
  ...
  def run(self):
    try:
      ...
      # 1. Torn Growth Pages & Truncated Table Chains[cite: 3]
      garbage_ec_pages = []
      ...
      # Truncated Table Chains Detection[cite: 3]
      truncated_chains = []
      for i in range(num_tables):
        toff = 0x1C + i * 16
        if toff + 16 > len(data_bytes):
      ...
      # 2. Sentinel u5 & Flags & B-Trees[cite: 3]
      ...
      self.progress.emit(75, "Applying corrective database repairs...")
      ...
      # Apply Truncated Table Chain Fixes[cite: 3]
      ...
      self.finished.emit(
          "export.pdb database successfully repaired & synced (Backup saved as"
          " export.pdb.bak)."
      )
...
if __name__ == "__main__":
  ...
  window.show()
  sys.exit(app.exec())import ctypes
import os
import shutil
import string
import struct
